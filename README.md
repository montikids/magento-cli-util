...
The tool is designed to automate, simplify, and make safe converting the production environment into a staging or a development one. It can be used by developers manually or as a part of a deployment proccess to configure the staging environment.

Actually, it allows describing a scenario of sanitizing data in the database, Thus you can be be sure you didn't forget to do something important, like disabling an integration locally (that can create a mess) or anonymizing customer emails (that could lead to accidental sending emails) and so on.
...
- Anonymizing customer sensitive data in Magento tables:
  - emails
  - names
  - addresses
  - etc.
- Replacing or deleting Magento store configuration values in the `core_config_data` table:
  ...
  - domain name
  - etc.
- Running custom SQL queries as a part of scenario
- Running specified [N98 Magerun 2](https://github.com/netz98/n98-magerun2) tool commands or Magento CLI commands as a part of scenario
- Flexible configuration per environment:
  ...
  - Use the local config (`local.yml`) to cover your personal needs: add, cusotmize or override any rules defined in the base and/or the environemnt config
- Specifying the verbosity flag `-v` allows you getting more detailed output or error informaton
...
After installation you must to configure the environment.
## Usage / Commands
...
- `dev` – is intended to be using for local development environment
- `stage` – is intended to be using for staging environment
...
> It's recommended to add the `mk-cli-util` folder to git in order to share configs across your team and have an ability to easy adjust the rules when a new feature creates some points to sanitize.


### Anonymize DB data

#### Description
Running this command starts data anonymizing processing in Magento database according to the scenario specified in the config(s).

Config paths:
- `mk-cli-util/config/anonymize/base.yml` – required, contains the main and the most common set of rules
- `mk-cli-util/config/anonymize/{{env_name}}.yml` – optional, the config name depends on the configured environment, contains rules are spicific for the environment
- `mk-cli-util/config/anonymize/local.yml` – optional, contains rules are spicific for you only, should neven be under VCS
...
For more verbosity (see SQL queries are executed, quantity of affected rows and so on)
```bash
...
- `mk-cli-util/config/store-config/{{env_name}}.yml` – optional, the config name depends on the configured environment, contains rules are spicific for the environment
- `mk-cli-util/config/store-config/local.yml` – optional, contains rules are spicific for you only, should neven be under VCS
...
The configs are merged in the way that allows inheriting from the basic configs and overriding values in the most specific one.
...
The `dev.yml` config is indended to be used across the development team locally.
...
Each developer or staging instance can have their own copy.
...
- If the same path is specified in more than one config, the value fromn the config with higher priority is used
  ...
  - the structure is the following
    ...
    tables:
      {{table_name}}
        ...
        {{field_name}}: null
    ```
    - `{{table_name}}` – is a valid table name in Magento database without he table prefix (if there is one)
    ...
    - `{{option_value}}` – depends on the option:
      - `value`
        ...
        - describes the column main value to replace with or the prefix, if you concat it with either another field value, postfix, or both
      ...
      - `is_password`
        - is a _boolean_ value, the default values is `false`
      ...
      - `concat_field_name`
        ...
        - I have no idea why someone would need it it but here we are
...
> That's because local developing requires

### Preparing
There is a trick to make composer load the local version of package instead of downloading it from the repository.
...
Unfortunatelly, in this case, the only solution that I found is to remove the `composer.lock` file.
Without this, the repository URL isn't updated and you continue receiving the package from the repository instaed of the specified local path.
```bash
rm -f composer.lock
```
...
Run installation of the packages and go to the kithchen to make yourself a cup of coffee
```bash
composer install
```
...
composer install
```
I prefer to run it as a single command, it's much more convinient when you need to test your changes quite often
...
> you would have to update the `composer.lock` in your Magento project. If
> ```bash
> composer update
> ```
> doesn't help, remove the `composer.lock` and run
> ```bash
> composer install
> ```
